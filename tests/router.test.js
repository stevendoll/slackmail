...
describe('POST /email', () => {
  ...
  it('handles markdown-wrapped JSON from Claude', async () => {
    ...
    mockFetch(true);
    const res = await request(app).post('/email').send({ subject: 'Your order' });
    expect(res.status).toBe(200);
...
describe('POST /slack', () => {
  it('returns 400 if channel is missing', async () => {
    const res = await request(app).post('/slack').send({ text: 'hello' });
    expect(res.status).toBe(400);
  });

  it('returns 400 if text is missing', async () => {
    const res = await request(app).post('/slack').send({ channel: 'inbox' });
    expect(res.status).toBe(400);
  });

  it('returns 400 for unknown channel', async () => {
    const res = await request(app).post('/slack').send({ channel: 'unknown', text: 'hi' });
    expect(res.status).toBe(400);
  });

  it('returns 502 if Slack fails', async () => {
    mockFetch(false);
    const res = await request(app).post('/slack').send({ channel: 'inbox', text: 'hello' });
    expect(res.status).toBe(502);
  });

  it('posts to slack and returns ok', async () => {
    mockFetch(true);
    const res = await request(app).post('/slack').send({ channel: 'inspo', text: 'Cool idea!' });
    expect(res.status).toBe(200);
